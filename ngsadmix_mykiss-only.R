...
## mykiss only (all ccgp + 10 new indvs from 8 weird pops)
...
ngsadmix_dir <- "ngsadmix/mykiss-only_filt_snps05-thin100_0-maf05" # set NGSadmix outputs directory
...
par(mfrow=c(4,2))
for(chosen_K in c(2,3,4,5,6,7,8)) {
...
# read metadata
metadata <- read.csv("refs/CCGP-trout_sample-coordinate-match_plus-10-new-samples.csv",header=T)

# merge sample list with metadata (in order of sample list)
sample_info <- merge(sample_list,metadata,by="sample_ID")

#order by population
#ord <- order(sample_info$subspecies_name)
ord <- order(sample_info$subspecies_name, sample_info$plotting_locality)

# organize by coordinate
#library(raster)
# set center
#center_lat <- 36
#center_long <- -118
#ord <- order(atan2(sample_info$latitude - center_lat, sample_info$longitude - center_long))
