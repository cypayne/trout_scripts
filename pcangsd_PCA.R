...
cov_file_path <- "coastal-and-hatchery-mykiss_only/coastal-and-hatchery-mykiss-only_og-ccgp.cov"
sample_list_path <- "coastal-and-hatchery-mykiss_only/sample_list.txt"
metadata_path <- "refs/CCGP-trout_sample-coordinate-match.csv"
...
pca <- ggplot(data = pca_data, aes(x = V1, y = V2, color=subspecies_common_name, label=sample_ID)) +
  geom_point(aes(color = subspecies_common_name), alpha = 0.8, size=2) + 
...
ggsave(pca, filename='Figures/coastal-and-hatchery-mykiss_pc1-pc2.pdf', device = cairo_pdf, width=14, height=6, bg="transparent")
