...
## coastal (steelhead) and hatchery mykiss only 
...
ngsadmix_dir <- "ngsadmix/mykiss-only_coastal-and-hatchery_snps05-thin100_0-maf05" # set NGSadmix outputs directory
...
# Inf   5132.5536         Inf  15112.0077  37895.9852 158316.1206  14032.2472    458.6543    216.3071         Inf 
# K=6 should be chosen
...
#pdf("Figures/mykiss-only_ccgp-trout_admix_plot_map-matched-colors_K2-8.pdf",width=14,height=10)
#par(mfrow=c(4,1))
for(k in c(2,3,4,5,6,7,8)) {
  
  ## plot
  pdf(paste0("Figures/coastal-hatchery-mykiss_admix_plot_map-matched-colors_K",k,".pdf"),width=14,height=10)
  ...
  for(rep in c(1:total_reps)) {
    ...
    #order by geography
    #ord <- order(sample_info$subspecies_name, sample_info$plotting_locality)
  ...
  # produce barplot
  #pdf("Figures/mykiss-only_ccgp-trout_admix_plot_map-matched-colors_K8.pdf",width=20,height=8)
  #par(mfrow=c(2,1))
  barplot(t(q_all_reps)[,ord],
          #        col=hue_pal()(chosen_K),
          col=trout_palette,
          names=sample_info$new_sample_name[ord],
          las=2,
          ...
          cex.names=0.8,
          xlab="",
          ylab=paste0("admixture proportions for K=",chosen_K)) + facet_wrap(~ file)
  #dev.off()
...
aesthetic_map <-
  ...
  geom_label(data=sample_coords_admix_nonNA_nohatchery, aes(x=longitude, y=latitude, group=region, label=sample_ID), 
             nudge_x=0.4, nudge_y=-0.1, label.size=0.05, size=1.2,  
